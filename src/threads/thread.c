...
void acquire_file_lock(){
  lock_acquire(&file_code_lock);
}

void release_file_lock(){
  lock_release(&file_code_lock);
}
#endif
...
void thread_priority_donate(struct thread*t){
  ...
  thread_update_priority(t); //cascade priority
...
void thread_update_priority(struct thread*t){
  //choose the max_priority of the lock thread is waiting for
  enum intr_level old_level = intr_disable();
  ...
  //O(n) for max_pri choose
  t->priority = max_pri;
  //printf("after release thread %s priority:%d\n",t->name,t->priority);
  intr_set_level(old_level);
}
...
thread_unblock (struct thread *t) 
{
  ...
  //if you add codes above before you finish priority-donate part there will be a deadlock,
  t->status = THREAD_READY;
  //printf("thread %s inserted back to ready list:\n",t->name);
  intr_set_level (old_level);
}
...
{
  /* Not yet implemented. */
  return FIXED_POINTS_ROUND(FIXED_POINTS_MUL_MIX(load_avg,100));
}
...
{
  /* Not yet implemented. */
  return FIXED_POINTS_ROUND(FIXED_POINTS_MUL_MIX(thread_current()->recent_cpu,100));
}
void mlfqs_RecentCpu_Inc(){
  ASSERT(thread_mlfqs);
  ASSERT(intr_context);
  struct thread * cur = thread_current();
  if (cur == idle_thread) //idle_thread is not included
      return;
  cur->recent_cpu = FIXED_POINTS_ADD_MIX(cur->recent_cpu,1);
}
...
void mlfqs_update_load_avg(){
  ...
  load_avg = FIXED_POINTS_ADD(FIXED_POINTS_DIV_MIX(FIXED_POINTS_MUL_MIX(load_avg,59),60),FIXED_POINTS_DIV_MIX(FIXED_POINTS_CONST(ready_cnt),60));
  ...
  for (e = list_begin(&all_list);e!=list_end(&all_list);e=list_next(e)){
      ...
      if (temp != idle_thread){
          //printf("temp.recent_cpu:%d\n",temp->recent_cpu);
          //int test2=FIXED_POINTS_DIV(FIXED_POINTS_MUL_MIX (load_avg, 2), \ 
					                   //FIXED_POINTS_ADD_MIX (FIXED_POINTS_MUL_MIX (load_avg, 2), 1));
          //int test =FIXED_POINTS_MUL(FIXED_POINTS_DIV(FIXED_POINTS_MUL_MIX (load_avg, 2), \ 
					                   //FIXED_POINTS_ADD_MIX (FIXED_POINTS_MUL_MIX (load_avg, 2), 1)), temp->recent_cpu);
          //printf("test:%d\n",test2);
          temp->recent_cpu = FIXED_POINTS_ADD_MIX (FIXED_POINTS_MUL(FIXED_POINTS_DIV(FIXED_POINTS_MUL_MIX (load_avg, 2), \ 
					                   FIXED_POINTS_ADD_MIX (FIXED_POINTS_MUL_MIX (load_avg, 2), 1)), temp->recent_cpu), temp->nice);
...
void mlfqs_update_priority(struct thread*t){
     ...
     t->priority = FIXED_POINTS_INVER(FIXED_POINTS_SUB_MIX(FIXED_POINTS_SUB(FIXED_POINTS_CONST(PRI_MAX),\
                                      FIXED_POINTS_DIV_MIX(t->recent_cpu,4)),2*t->nice));
...
init_thread (struct thread *t, const char *name, int priority)
{
  ...
  t->recent_cpu = FIXED_POINTS_CONST(0);
  t->nice = 0;
  ...
  t->max_file_fd = 2;//fd = 0和fd =1 fd=2由pintos保留
  list_init(&t->childs);
  list_init(&t->files);
  sema_init(&t->sema,0);
  t->success = true;
